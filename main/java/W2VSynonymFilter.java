...
public final class W2VSynonymFilter extends TokenFilter {
  ...
  public W2VSynonymFilter(TokenStream input, Word2Vec word2Vec, double minAccuracy) { //creates a token filter with input a trained model of w2v
    super(input);
  ...
  public boolean incrementToken() throws IOException {
    ...
    if (!SynonymFilter.TYPE_SYNONYM.equals(typeAtt.type())) { 
      ...
      for (String syn : list) {
        if (i == 2) {
          break; //mono 2 sinonima ana token
        }
        if (!syn.equals(word)) {
